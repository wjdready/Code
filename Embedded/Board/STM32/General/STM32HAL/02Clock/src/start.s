...
/* .data section 的初始化值的起始地址，在链接器脚本中定义 */
.word  _sidata
...
/**
 * @brief  这是处理器在重置事件后首次开始执行时调用的代码。
  *        只执行绝对必要的设置，然后调用应用程序提供的main()例程
 * @param  None
 * @retval : None
*/
...
/* 定义 myasfunc 标签属性: 全局函数 */
.type myasfunc, %function
.global myasfunc
...
/**
 * @brief  这是处理器收到意外中断时调用的代码。这只是进入无限循环，保留系统状态以供调试器检查。
 * @param  None
 * @retval None
*/
...
/******************************************************************************
*
* Cortex M的最小向量表。请注意，必须在其上放置适当的构造，以确保它最终到达物理地址0x0000.0000。
*
*******************************************************************************/
...
/*******************************************************************************
*
* 向Default_Handler提供每个异常处理程序的弱别名。
* 由于它们是弱别名，因此任何同名函数都将覆盖此定义。
*
*******************************************************************************/
   .weak      NMI_Handler
