...
if __name__ == '__main__':
    ...
    # 如果包含 MicNode 则获取完整名
    allname = sd.query_devices()
    for dev in allname:
        if 'MicNode' in dev['name']:
            device = dev['name']
            break
    device += ', MME'
    print("Final device:", device)
    myrecording = sd.rec(int(duration * fs), samplerate=fs, channels=1, dtype='int32', device=device)
    print('Waiting...')
    sd.wait()  # Wait until recording is finished
    print('Done!')

    # 保存到wav
    sf.write('MicNode.wav', myrecording, fs, subtype='PCM_32')
