...
#ifndef UW_SELECTBOX_H
#define UW_SELECTBOX_H
...
#if LV_USE_LABEL == 0
#error "uw_selectbox: lv_label is required. Enable it in lv_conf.h (LV_USE_LABEL 1)"
#endif

/*********************
 *      DEFINES
 *********************/
#define UW_SELECTBOX_POS_LAST 0xFFFF
LV_EXPORT_CONST_INT(UW_SELECTBOX_POS_LAST);
...
typedef struct {
    lv_obj_t obj;
    lv_obj_t * cont_hint;   /* 显示提示部分 */
    lv_obj_t * cont_list;   /* 用于显示列表的容器 */
    lv_obj_t * dropdown;
} uw_selectbox_list_t;

extern const  lv_obj_class_t uw_selectbox_class;
extern const  lv_obj_class_t uw_selectboxlist_class;
 ...
 * @param pos       the insert position, indexed from 0, UW_SELECTBOX_POS_LAST = end of string
 */
