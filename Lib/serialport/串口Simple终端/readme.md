---
mytags: myblog
title: 使用 libserialport 来实现简单的串口终端
date: 2023-02-15 10:59:47
categories: [Lib, serialport]
tags: [串口]
---

本文主要介绍使用 libserialport 来实现简单的串口终端

<!-- more -->

如下所示, 打开程序后首先查找可用的连接串口并尝试连接, 然后开新的线程用来在终端中获取每一行输入, 发送到串口, 在主线程中不断接收的串口数据并打印
...
```makefile
default:
	gcc serial_terminal.c -lserialport -lpthread -lCfgmgr32 -lSetupapi -static 
